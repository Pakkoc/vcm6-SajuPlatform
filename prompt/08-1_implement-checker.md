...
1. /docs/{requirement,persona,prd,userflow,database,common-modules,test-plan}.md, /docs/rules/tdd.md문서를 읽고 프로젝트의 기획을 구체적으로 파악한다.
2. 해당 서비스의 연동 가이드 문서 경로를 파악하고, 내용을 읽어들여 구체적으로 파악한다.
3. /docs/usecases/N-name/{spec}.md 경로로부터 해당 서비스와 연관된 기능들의 유스케이스를 상세히 파악한다.
4. /docs/pages/N-name/{plan,state}.md 경로로부터 해당 서비스와 연관된 페이지들의 구현 계획을 상세히 파악한다.
